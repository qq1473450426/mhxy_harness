...
class GameRunner:
    ...
    def stop(self):
        ...
        self._running=False
    def _prepare_coordinator(self, win=None):
    ...
    def _run_attached(self,win,task,goal,auto):
        try:
            ...
            self.coordinator=self._prepare_coordinator(win); self._phase="team"; self.coordinator.bind_task(task); self.rl_env=self._build_rl_env(); self._phase="running"
            if auto: self.coordinator.start(goal=goal or task,max_steps=0)
    ...
    def _run(self,task,goal,auto):
        try:
            if self.dry_run: self.coordinator=self._prepare_coordinator(); self._phase="running"
            else:
                ...
                self.coordinator=self._prepare_coordinator()
            self._phase="team"; self.coordinator.bind_task(task); self.rl_env=self._build_rl_env(); self._phase="running"
            if auto: self.coordinator.start(goal=goal or task,max_steps=0)
