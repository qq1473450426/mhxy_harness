...
class GameRunner:
    ...
    def start_attached(self,win:WindowInfo,task="shimen",goal="",auto=True):
        ...
        try:
            self._stop.clear();self._phase="attaching";self.client.state.window_hwnd=win.hwnd;self.client.state.window_title=win.title;self.client.state.running=True;self.client.state.phase="in_game";self.coordinator=self._prepare_coordinator(win,task);leader=self.coordinator.team.leader
            if not leader or leader not in self.coordinator.agents:self.coordinator=None;self._phase="error:no_leader_agent";return False
            self._last_status=self.coordinator.status();self._running=True;self._thread=threading.Thread(target=self._run_attached,args=(win,task,goal,auto),daemon=True);self._thread.start();return True
        except Exception as exc:logger.exception("attached initialization failed");self._phase="error:"+str(exc)[:120];self._running=False;return False
    def step_attached(self,win:WindowInfo,task="shimen",goal=""):
        ...
        try:
            ...
            if self.coordinator is None:self.coordinator=self._prepare_coordinator(win,task);self._phase="attached_step"
            ...
            result=agent.step(goal or task);self._last_status=self.coordinator.status();return {"ok":True,"result":result}
        except Exception as exc:logger.exception("single step failed");return {"ok":False,"error":str(exc)}
    def stop(self):
        self._stop.set()
        ...
        self._running=False
    def _prepare_coordinator(self,win=None,task_name=""):
        c=Coordinator(self.settings,ocr_backend=self.settings.get("vision",{}).get("ocr_backend","rapidocr"),tick_seconds=float(self.settings.get("automation",{}).get("tick_seconds",0.5)));c.build_from_config()
        if win is None:c.bind_windows()
        else:
            leader=c.team.leader
            if not leader and c.accounts:leader=next(iter(c.accounts));c.team.set_leader(leader)
            if leader and leader in c.accounts:
                ...
                inp=self.settings.get("input",{});acc.driver=InputDriver(win,backend=inp.get("backend","win32"),failsafe=bool(inp.get("failsafe",True)),move_duration=float(inp.get("move_duration",0.08)))
    ...
    def start_learning(self,win:WindowInfo)->Dict[str,Any]:
        ...
        try:
            self.learning_recorder=HumanDemoRecorder(win,root="learning/demos",sample_hz=4.0);path=self.learning_recorder.start();self._phase="human_learning";self._learning_result={"ok":True,"status":"recording","path":path,"actions":0};return self._learning_result
        except Exception as exc:logger.exception("human learning start failed");return {"ok":False,"error":str(exc)}
    def stop_learning(self,success:bool=True)->Dict[str,Any]:
        ...
        try:
            path=self.learning_recorder.stop();count=len(self.learning_recorder.actions);self.learning_recorder._write_meta({"success":bool(success)});self._learning_result={"ok":True,"status":"completed","success":bool(success),"path":path,"actions":count};self._phase="idle";return self._learning_result
        except Exception as exc:return {"ok":False,"error":str(exc)}
    def train_learning(self,passes:int=8)->Dict[str,Any]:
        ...
        policy.save("learning/policy.json");self.learning_policy=policy;stats.update({"demos":demos,"loaded":loaded,"policy":"learning/policy.json"});self._learning_result=stats;return stats
    ...
    def status(self):return {"phase":self._phase,"running":self._running,"dry_run":self.dry_run,"client":self.client.to_dict(),**self._last_status,"rl":self.rl_env.to_dict() if self.rl_env else {},"optimize":self._last_optimize,"paused":self.paused_accounts(),"battle":self._battle_stats,"learning":self.learning_status()}
