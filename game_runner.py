...
class GameRunner:
    ...
    def start(self, task="shimen", goal="", auto=True):
        ...
        self._stop.clear()
        self._running = True
        self._thread = threading.Thread(target=self._run, args=(task, goal, auto), daemon=True)
        self._thread.start()
        return True

    def start_attached(self, win: WindowInfo, task="shimen", goal="", auto=True):
        if self._running or win is None or not win.is_valid():
            return False
        self._stop.clear()
        self._running = True
    ...
    def _prepare_coordinator(self, win=None, task_name: str = "", goal: str = ""):
        c = Coordinator(
            self.settings,
            ocr_backend=self.settings.get("vision", {}).get("ocr_backend", "rapidocr"),
            tick_seconds=float(self.settings.get("automation", {}).get("tick_seconds", 0.5)),
        )
        c.build_from_config()
        if win is None:
            c.bind_windows()
            ...
            if leader and leader in c.accounts:
                acc = c.accounts[leader]
                acc.win = win
                acc.state.hwnd = win.hwnd
                acc.state.win_rect = win.rect
                from automation.input_driver import InputDriver
                inp = self.settings.get("input", {})
                acc.driver = InputDriver(
                    win,
                    backend=inp.get("backend", "win32"),
                    failsafe=bool(inp.get("failsafe", True)),
                    move_duration=float(inp.get("move_duration", 0.08)),
                )
            c.team.members = [leader] if leader else []
            c.team.team_ready = bool(c.team.members)
        if task_name:
            bound = c.bind_task(task_name, goal=goal)
            if bound is None:
                raise RuntimeError(f"无法绑定任务: {task_name}")
        c.create_agents()
        return c
    ...
    def _run_attached(self, win, task, goal, auto):
        try:
            ...
            self.coordinator = self._prepare_coordinator(win, task, goal)
            self._phase = "team"
            self.rl_env = self._build_rl_env()
            self._phase = "running"
            if auto:
                self.coordinator.start(goal=goal or task, max_steps=None)
            while not self._stop.is_set():
                self._last_status = self.coordinator.status()
                time.sleep(0.5)
        except Exception as exc:
            logger.exception("attached runner error")
    ...
    def _run(self, task, goal, auto):
        try:
            if self.dry_run:
                self.coordinator = self._prepare_coordinator(task_name=task, goal=goal)
                self._phase = "running"
            else:
    ...
    def _build_rl_env(self):
        ...
        def observe_fn():
            return {"team": coord.team.to_dict(), "accounts": {aid: a.to_dict() for aid, a in coord.accounts.items()}}

        def act_fn(action):
            agent = coord.agents.get(coord.team.leader)
            if agent is None or not isinstance(action, dict):
                return {"ok": False}
            ...
            try:
                ar = agent.act(d, agent.observe())
                return {"ok": bool(ar.ok), "desc": ar.desc, "error": ar.error}
            except Exception as exc:
                return {"ok": False, "error": str(exc)}

        return ProxyGameEnvironment(
            observe_fn,
            act_fn,
            lambda obs: 1.0 if obs.get("team", {}).get("status") == "TASKING" else 0.0,
        )
    ...
    def init_battle_monitor(self):
        try:
            ...
            logger.warning("战斗监控初始化失败: %s", exc)
        return False

    def handle_battle(self):
        if self._battle_monitor is None:
            return {"battled": False, "reason": "未初始化"}
        self._battle_stats = self._battle_monitor.monitor_once(max_wait=150.0)
        return self._battle_stats

    def pause_account(self, account_id):
        return bool(self.coordinator and self.coordinator.pause_account(account_id))

    def resume_account(self, account_id):
        return bool(self.coordinator and self.coordinator.resume_account(account_id))

    def paused_accounts(self):
        return self.coordinator.paused_accounts() if self.coordinator else []
    ...
    def optimize(self, episodes=300):
        try:
            ...
            stats = trainer.train()
            stats["opt_steps"] = trainer.evaluate()
